                ...
                @foreach ($products as $p)
                    <div class="px-4 relative card group">
                        <div class="rounded-xl overflow-hidden card-shadow relative" style="width: 287px; height: 386px">
                            ...
                            <img src="{{ $p->gallery()->exists() ? Storage::url($p->gallery->first()->url) : '' }}"
                                alt="" class="w-full h-full object-cover object-center" />
                        </div>
                        <h5 class="text-lg font-semibold mt-4">{{ $p->title }}</h5>
                        <span class="">{{ number_format($p->price) }}</span>
                        <a href="{{ route('details', $p->slug) }}" class="stretched-link">
                            <!-- fake children -->
                        </a>
                    </div>
                @endforeach
