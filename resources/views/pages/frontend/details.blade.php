    ...
    <section class="container mx-auto">
        <div class="flex flex-wrap my-4 md:my-12">
            <div class="w-full md:hidden px-4">
                <h2 class="text-5xl font-semibold">{{ $products->title }}</h2>
                ...
                <div class="slider">
                    ...
                    <div class="preview">
                        <div class="item rounded-lg h-full overflow-hidden">
                            <img src="{{ $products->gallery()->exists() ? Storage::url($products->gallery->first()->url) : '' }}"
                                class="object-cover w-full h-full rounded-lg" />
                        </div>
                    </div>
                </div>
                    ...
                    <div class="px-3 flex-none" style="width: 320px">
                        <div class="rounded-xl p-4 pb-8 relative bg-white">
                            <div class="rounded-xl overflow-hidden card-shadow w-full h-36">
                                <img src="{{ $recom->gallery()->exists() ? Storage::url($recom->gallery->first()->url) : '' }}"
                                    alt="" class="w-full h-full object-cover object-center" />
                            </div>
                            <h5 class="text-lg font-semibold mt-4">{{ $recom->title }}</h5>
                            <span class="">IDR {{ number_format($recom->price) }}</span>
                            <a href="{{ route('details', $recom->slug) }}" class="stretched-link">
                                <!-- fake children -->
                            </a>
                        </div>
                    </div>
